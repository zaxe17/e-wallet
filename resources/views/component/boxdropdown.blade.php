<div class="show input-shadow opacity-0 bg-transparent flex flex-col px-9 py-3 rounded-lg">
    <!-- HEADER -->
    <div class="flex justify-start items-center gap-3">
        ...
        <div class=" text-2xl lato-normal flex items-center">
            <span class="font-extrabold">{{ $boxName }} for </span>
            ...
            <form method="GET" action="{{ request()->routeIs('expenses.*') ? route ('expenses.index') : route('earnings.index') }}">
                <select name="month" id="month"
                    ...
                    <option value="{{ $num }}"
                        {{ request('month', $currentMonth) == $num ? 'selected' : '' }}>
                        {{ $name }}
                    </option>
                    @endforeach
                </select>
