        ...
        <div class="bg-[#F5F5F5]/80 form-shadow rounded-2xl px-28 py-11 backdrop-blur-sm">
            ...
            <form id="addForm" action="" class="flex flex-col gap-5">
                ...
                <div class="flex justify-between items-center gap-16">
                    ...
                    <input oninput="capitalizeInput(this)" type="{{ $field['type'] }}" name="{{ $field['name'] }}" id="{{ $field['name'] }}" class="w-3xs h-8 bg-white/30 backdrop-blur-[15px] rounded-md font-medium input-shadow px-2 py-1 text-sm focus:outline-none">
                </div>
                @endforeach
